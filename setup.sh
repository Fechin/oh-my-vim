...
if hash apt-get 2>/dev/null; then
    sudo apt-get install -y build-essential cmake python-dev pyflakes node npm markdown git libxml2
elif hash yum 2>/dev/null; then
    sudo yum install -y build-essential cmake python-dev pyflakes node markdown git libxml2
fi

# Mac 系统安装
if hash brew 2>/dev/null; then
    sudo easy_install pyflakes
    brew install cmake node markdown libxml2
fi
